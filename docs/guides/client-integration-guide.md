...
Use `createFromRegistry({ agentId })` for the normal host path, and only drop to raw stdio process management when building a lower-level ACP client or transport adapter.
## Translation
