...
如果你接入的是 `acp-runtime`，而不是直接操作 raw ACP transport，
推荐优先使用 runtime 的 registry 入口：
...
这样 agent 启动解析会集中维护在 runtime 里，而不是每个宿主自己重复维护 `command` / `args` 逻辑。
...
默认推荐走 `runtime.sessions.start({ agent })`，其中 `agent` 可以是 registry agent id。
...
- [client-sdk-demo.ts](/Users/dev/acp-runtime/examples/client-sdk-demo.ts)
