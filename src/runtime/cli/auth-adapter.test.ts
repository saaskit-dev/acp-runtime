...
import {
  ...
  resolveRuntimeCliTerminalAuthenticationRequest,
...
describe("runtime CLI auth adapter", () => {
  it("consumes Claude profile login success patterns", () => {
    const request = resolveRuntimeCliTerminalAuthenticationRequest({
      agent: {
        command: "claude",
        type: "claude-acp",
      },
  ...
  it("uses SDK metadata to select the default authentication method", async () => {
    ...
    try {
      const result = await promptForRuntimeCliAuthentication({
        inputCoordinator: {
          close: vi.fn(),
