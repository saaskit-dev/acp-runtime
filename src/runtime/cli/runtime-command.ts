...
import { promptForRuntimeCliAuthentication } from "./auth-adapter.js";
import {
  configureRuntimeCliLogSink,
  type RuntimeCliLogSink as LogSink,
} from "./log-sink.js";
import {
  createInputCoordinator,
  createOutputGate,
  type RuntimeCliInputCoordinator as InputCoordinator,
  type RuntimeCliOutputGate as OutputGate,
} from "./input.js";
import {
  findSimilarLocalSessionReferences,
  resolveRuntimeCliConfig,
} from "./command-config.js";
import { parseCliOptions } from "./command-options.js";
...
function formatOperationHeader(operation: AcpRuntimeOperation): string {
  return shortenForTerminal(formatZedToolLabel(operation), 110);
}
...
function summarizeOperationSubject(operation: AcpRuntimeOperation): string {
  const label = formatZedToolLabel(operation);
  if (label) {
    return shortenForTerminal(label, 96);
  }
...
function formatZedToolLabel(operation: AcpRuntimeOperation): string {
  const title = operation.title.trim();
  if (operation.kind === AcpRuntimeOperationKind.ExecuteCommand) {
    return title;
  }

  const singleLine = firstDisplayLine(title);
  if (operation.kind === AcpRuntimeOperationKind.WriteFile) {
    return escapeMarkdown(singleLine);
  }
  return singleLine;
}
...
function formatZedToolStatus(operation: AcpRuntimeOperation): string {
...
function formatOperationInline(
  ...
  const status = formatZedToolStatus(operation);
  ...
  return `${subject} | ${status} | ${errorMessage ? shortenForTerminal(errorMessage, 96) : "Failed"}`;
}

function formatOperationDetail(operation: AcpRuntimeOperation): string {
  const lines = [
    formatOperationHeader(operation),
    `status=${formatZedToolStatus(operation)}`,
    `kind=${describeOperationKind(operation.kind)}`,
  ];
...
function hasOperationResultDetails(operation: AcpRuntimeOperation): boolean {
  return Boolean(
    operation.result?.summary ||
      operation.result?.outputText?.trim() ||
      operation.result?.output?.length ||
...
function summarizeMetadata(
  ...
  if (sessionMetadata.title) {
    parts.push(`title=${sessionMetadata.title}`);
  }
  if (sessionMetadata.currentModeId) {
    parts.push(`mode=${sessionMetadata.currentModeId}`);
  }
  if (
  ...
  if (sessionMetadata.availableCommands) {
    parts.push(`commands=${sessionMetadata.availableCommands.length}`);
  }
  if (sessionMetadata.updatedAt) {
    parts.push(`updated=${sessionMetadata.updatedAt}`);
  }

  return parts.join(" | ");
}

function formatPlanStatus(status: AcpRuntimePlanItem["status"]): string {
  switch (status) {
    case "completed":
      return "[x]";
    case "in_progress":
...
function summarizePlan(plan: readonly AcpRuntimePlanItem[]): string {
  const completed = plan.filter((entry) => entry.status === "completed").length;
  const inProgress = plan.find((entry) => entry.status === "in_progress");
  const pending = plan.filter((entry) => entry.status !== "completed").length;

  if (plan.length === 0) {
    return "Plan | empty";
  }
  if (inProgress) {
    return `Current: ${shortenForTerminal(inProgress.content, 96)} | ${pending} left`;
  }
  if (pending === 0) {
    return `Plan | All Done | ${completed}/${plan.length}`;
  }
  if (completed === 0) {
    return `Plan | ${plan.length} Tasks`;
  }
  return `Plan | ${completed}/${plan.length}`;
}

function formatPlan(plan: readonly AcpRuntimePlanItem[]): string {
  const lines = [summarizePlan(plan)];
  for (const entry of plan) {
    lines.push(
      `${formatPlanStatus(entry.status)} ${shortenForTerminal(entry.content, 110)}`,
    );
  }
  return lines.join("\n");
}
...
function formatUsage(usage: {
  ...
  if (
    ...
    const ratio =
      ...
      usage.contextWindowTokens > 0
        ? ` (${Math.round((usage.contextUsedTokens / usage.contextWindowTokens) * 100)}%)`
        : "";
    parts.push(
      `ctx=${typeof usage.contextUsedTokens === "number" ? formatCount(usage.contextUsedTokens) : "?"}/${typeof usage.contextWindowTokens === "number" ? formatCount(usage.contextWindowTokens) : "?"}${ratio}`,
    );
  }
  ...
  if (typeof usage.costUsd === "number" && usage.costUsd > 0) {
    parts.push(`cost=${formatUsd(usage.costUsd)} USD`);
  }
...
function formatEventDetail(event: AcpRuntimeTurnEvent): string {
  switch (event.type) {
    ...
    case AcpRuntimeTurnEventType.PermissionRequested:
      return `Waiting for confirmation | ${event.request.kind} | ${shortenForTerminal(event.request.title, 96)}`;
    case AcpRuntimeTurnEventType.PermissionResolved:
      return `${event.decision === "allowed" ? "Allowed" : "Rejected"} | ${event.request.kind} | ${shortenForTerminal(event.request.title, 96)}`;
    case AcpRuntimeTurnEventType.PlanUpdated:
      return formatPlan(event.plan);
...
function createTimelineRenderer(
  ...
  function createScopedTurnRenderer(
    ...
    function updateThinkingStatus(): void {
      ...
      const clock = formatClock(new Date());
      const elapsed = formatElapsed(turnStartedAt);
      const frame = frames[thinkingFrame % frames.length] ?? "thinking";
      thinkingFrame += 1;
      outputGate.setStatus(`${clock}  ${elapsed}  thinking  ${frame}`);
    }

    function startThinkingStatus(): void {
      if (thinkingTimer) {
        return;
      }
      updateThinkingStatus();
      thinkingTimer = setInterval(updateThinkingStatus, 300);
    }

    function stopThinkingStatus(): void {
      if (thinkingTimer) {
        clearInterval(thinkingTimer);
        thinkingTimer = undefined;
      }
      outputGate.clearStatus();
    }
  ...
  return {
    ...
    project(event): TurnEventProjectionInstruction {
      ...
      if (event.type === AcpRuntimeTurnEventType.MetadataUpdated) {
        ...
        if (!detail || detail === metadataDetail) {
          return { kind: "skip" };
        }
      ...
      if (event.type === AcpRuntimeTurnEventType.PlanUpdated) {
        return {
          detail: formatEventDetail(event),
          kind: "line",
          label: "plan",
      ...
      if (
        ...
        if (
          ...
          !hasOperationResultDetails(event.operation)
        ) {
          return { kind: "skip" };
        }
        const detail =
  ...
  while (true) {
    ...
    if (answer === "/exit" || answer === "/quit") {
      ...
      logSink.emit({
        ...
        body: request.title,
        eventName: "acp.runtime_cli.permission.resolved",
      ...
      try {
        ...
        logSink.emit({
          attributes: {
            ...
            "acp.runtime_cli.command.name": "mode",
          },
        ...
        logSink.emit({
          ...
          body: formatted,
          eventName: "acp.runtime_cli.local_command.failed",
          exception: error,
      ...
      try {
        ...
        logSink.emit({
          attributes: {
            ...
            "acp.runtime_cli.command.name": "config_set",
...
async function main(): Promise<void> {
  ...
  const config = await resolveRuntimeCliConfig(runtime, options);
  if (!config && !options.listSessions) {
    const sessionId = options.loadSessionId ?? options.resumeSessionId;
    const similar = sessionId
      ? await findSimilarLocalSessionReferences(runtime, sessionId)
      : [];
    const suggestion = similar.length
      ? ` Did you mean ${similar
          .map((session) =>
            `${session.id}${session.agentType ? ` (${session.agentType})` : ""}`,
          )
          .join(", ")}?`
      : "";
    throw new Error(
      sessionId
        ? `[runtime] no local stored session found for ${sessionId}; pass an agent explicitly if this is remote-only, or check the session id.${suggestion}`
        : "[runtime] no agent was provided.",
    );
  }
  const logSink = await configureRuntimeCliLogSink(options.logFile, {
  ...
  const createPermissionHandler =
    () => (request: AcpRuntimePermissionRequest) => {
      logSink.emit({
        ...
        body: request.title,
        eventName: "acp.runtime_cli.permission.prompted",
      });
  ...
  const createAuthenticationHandler =
    ...
    (
      ...
      promptForRuntimeCliAuthentication({
        inputCoordinator,
        logSink,
        renderer,
        request,
  ...
  if (options.resumeLast) {
    ...
    if (!latestSessionId) {
      throw new Error(
        config
          ...
          : "[runtime] no locally persisted sessions found",
      );
    }
    ...
    session = await runtime.sessions.resume({
      ...(options.agentId && config
        ? { agent: config.agent, cwd: config.cwd }
        : {}),
      handlers: {
        authentication: createAuthenticationHandler(),
        ...(config?.handlers ?? {}),
    ...
    if (!config) {
      throw new Error("[runtime] no agent was provided.");
    }
  ...
  if (initialConfigReport) {
    ...
    logSink.emit({
      attributes: {
        "acp.runtime_cli.initial_config.ok": Boolean(
          session.initialConfigReport?.ok,
        ),
        "acp.runtime_cli.initial_config.items":
          session.initialConfigReport?.items.length ?? 0,
      },
      body: initialConfigReport,
      eventName: "acp.runtime_cli.initial_config",
  ...
  const displayAgentId = config?.agentId ?? readySnapshot.agent.type ?? "stored";
  const displayLabel = config?.label ?? displayAgentId;
  const displayCwd = config?.cwd ?? readySnapshot.cwd;
  logSink.emit({
    attributes: {
      "acp.agent.id": displayAgentId,
      "acp.agent.type": readySnapshot.agent.type,
      "acp.runtime_cli.label": displayLabel,
      "acp.session.id": session.metadata.id,
      "acp.session.cwd": displayCwd,
    },
    body: "Runtime CLI session ready.",
    eventName: "acp.runtime_cli.session.ready",
  });

  console.log("[runtime] session created");
  console.log(`[runtime] sessionId: ${session.metadata.id}`);
  console.log(`[runtime] agentId: ${displayAgentId}`);
  console.log(`[runtime] label: ${displayLabel}`);
  console.log(
    `[runtime] agentType: ${readySnapshot.agent.type ?? "<none>"}`,
  );
  console.log(`[runtime] cwd: ${displayCwd}`);
  if (process.platform === "win32" && displayAgentId === "codex-acp") {
    ...
    if (session) {
      try {
        ...
        logSink.emit({
          ...
          eventName: "acp.runtime_cli.session.close.failed",
          severityNumber: SeverityNumber.WARN,
        });
      }
    }
