...
import type { RuntimeCliInputCoordinator } from "./input.js";
import type { RuntimeCliLogSink } from "./log-sink.js";

type RuntimeCliTimelineRenderer = {
  writeLine(label: string, detail: string): void;
};

type RuntimeCliTerminalAuthenticationRequest = AcpRuntimeTerminalAuthenticationRequest & {
  successPatterns?: readonly string[];
};

export async function promptForRuntimeCliAuthentication(
  input: {
    inputCoordinator: RuntimeCliInputCoordinator;
    logSink: RuntimeCliLogSink;
    renderer: RuntimeCliTimelineRenderer;
    request: Parameters<
  ...
  if (!method) {
    input.logSink.emit({
      ...
      eventName: "acp.runtime_cli.authentication.cancelled",
      severityNumber: SeverityNumber.WARN,
    });
    return { cancel: true };
  }

  const terminalRequest = resolveRuntimeCliTerminalAuthenticationRequest({
  ...
  if (terminalRequest) {
    input.logSink.emit({
      ...
      eventName: "acp.runtime_cli.authentication.started",
    });
    ...
    throw new Error(
      `Authentication method "${method.title}" requires env vars and is not supported by this runtime CLI yet.`,
    );
  }
  ...
  input.logSink.emit({
    ...
    body: method.title,
    eventName: "acp.runtime_cli.authentication.selected",
  });
  return { methodId: method.id };
}

export function resolveRuntimeCliTerminalAuthenticationRequest(input: {
  agent: Parameters<
    NonNullable<AcpRuntimeAuthorityHandlers["authentication"]>
  >[0]["agent"];
  method: AcpRuntimeAuthenticationMethod;
}): RuntimeCliTerminalAuthenticationRequest | undefined {
...
async function runTerminalAuthentication(
  rl: Interface,
  request: RuntimeCliTerminalAuthenticationRequest,
): Promise<void> {
