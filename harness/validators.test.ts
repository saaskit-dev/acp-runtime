...
describe("harness validators", () => {
  it("accepts a valid harness case", () => {
    const value = {
      ...
      probes: {
        "codex-acp": {
          modeId: "read-only",
          prompt: "Write ./.tmp/tmp-output.txt",
        },
      },
      steps: [
        { type: "initialize" },
        {
          type: "session-prompt",
          prompt: "$probe-prompt",
          defaultPrompt: "Write ./.tmp/tmp-output.txt",
        },
      ],
  ...
  it("accepts a scenario category on harness cases", () => {
    expect(isHarnessCase({
      version: 1,
  ...
  it("accepts composite tool-kind assertions", () => {
    expect(isHarnessCase({
      version: 1,
      id: "scenario.read-file",
      kind: "scenario",
